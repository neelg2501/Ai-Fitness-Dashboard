...
# ── DASHBOARD WINDOW: UNLOCKED CONTENT ───────────────────────────
    ...
    name = st.sidebar.text_input("Your Name", value=st.session_state.current_user.capitalize())
    ...
    if not submit_btn:
        st.info(f"👋 Welcome, {st.session_state.current_user.capitalize()}! Please fill in your health parameters in the sidebar on the left and click **'Generate AI Strategy'** to view your personalized dashboard report.")
        
        st.write("---")
        st.subheader("💡 System Modules Included")
        st.checkbox("Phase 0 – Authentication & Secure Gatekeeper Layer", value=True, disabled=True)
...
| Progress Tracker | LSTM Time Series | Prediction | {lstm_next_wt} kg |
            ...
            st.markdown("---")
            st.success("✅ Phase 0 (Auth) + Phase 1 (Input) + Phase 2 (ML Models) + Phase 3 (Dashboard) — Complete!")
